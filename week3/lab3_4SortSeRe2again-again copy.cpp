...
int main()
{
    ...
    {
        int currentccurrence = 1;
        while (i < size - 1 && newArr[i] == newArr[i + 1])
        {
            currentccurrence++;
            i++;
        }

        if (currentccurrence > maxccurrence)
        {
            maxccurrence = currentccurrence;
            dup = to_string(newArr[i]);
        }
        else if (currentccurrence == maxccurrence)
        {
            dup += to_string(newArr[i]);
        }
    }
